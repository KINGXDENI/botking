...
const { mediamenu } = require('./lib/mbaru')
const { daily, level, register, afk, reminder, premiu, limits} = require('./function')
//////////////////////////////FOLDER SYSTEM///////////////////////////////////
...
module.exports = HandleMsg = async (piyo, message) => {
    try {
        ...
        const StickerNobg = { author: authorstc, pack: packstc, keepScale: true , removebg: true}
		const StickerNobgcrop = { author: authorstc, pack: packstc, keepScale: false , removebg: true}
        const cecan = [
            {
        ...
        // Log
        if (isCmd && !isGroupMsg && !isBanned) console.log(color('[BOT]'), color(time, 'yellow'), color(`${command} [${args.length}]`), 'from', color(pushname))
        if (isCmd && isGroupMsg && !isBanned) console.log(color('[BOT]'), color(time, 'yellow'), color(`${command} [${args.length}]`), 'from', color(pushname), 'in', color(name || formattedTitle))
        ...
        function isStickerMsg(id) {
            ...
            for (let i of stickerspam) {
                if (i.id === id) {
                    if (i.msg >= 5) {
                        found === true
                        piyo.reply(from, '*「 𝗔𝗡𝗧𝗜 𝗦𝗣𝗔𝗠 𝗦𝗧𝗜𝗖𝗞𝗘𝗥 」*\nKamu telah SPAM STICKER di grup, kamu akan di kick otomatis oleh DenBot', message.id).then(() => {
        ...
        //////////////
        if (isAutoStikerOn && isMedia && isImage || isQuotedImage && !isCmd) {
            const encryptMedia = isQuotedImage ? quotedMsg : message
            const _mimetype = isQuotedImage ? quotedMsg.mimetype : mimetype
            const mediaData = await decryptMedia(encryptMedia, uaOverride)
            const imageBase64 = `data:${_mimetype};base64,${mediaData.toString('base64')}`
            await piyo.sendImageAsSticker(from, imageBase64, StickerMetadatacrop)
                .then(async () => {
        ...
        if (isAutoStikerOn && isMedia && isVideo && !isCmd) {
            const mediaData = await decryptMedia(message, uaOverride)
            const videoBase64 = `data:${mimetype};base64,${mediaData.toString('base64')}`
            await piyo.sendMp4AsSticker(from, videoBase64, gifcrop ,StickerMetadatacrop)
                .then(async () => {
                    console.log(color(`Sticker processed for ${processTime(t, moment())} seconds`, 'aqua'))
                    })
                .catch(async (err) => {
        ...
        if (!gg && !hh) {
            switch (command) {
                ...
                case 'systemmenu':
                    piyo.sendText(from, systemmenu(prefix))
                    break
                case 'mediamenu':
                    piyo.sendText(from, mediamenuu(prefix))
                    break
                case 'toolmenu':
                    piyo.sendText(from, toolmenu(prefix))
                    break
                case 'animemenu':
                    piyo.sendText(from, animemenu(prefix))
                    break
                case 'nsfwmenu':
                    piyo.sendText(from, menu18(prefix))
                    break
                case 'nulismenu':
                    piyo.sendText(from, nulismenu(prefix))
                    break
                case 'tobatmenu':
                    piyo.sendText(from, tobatmenu(prefix))
                    break
                case 'stickermenu':
                    piyo.sendText(from, stickermenu(prefix))
                    break
                case 'imagemenu':
                    piyo.sendText(from, imagemenu(prefix))
                    break
                case 'logomenu':
                    piyo.sendText(from, logomenu(prefix))
                    break
                case 'gamemenu':
                    piyo.sendText(from, gamemenu(prefix))
                    break
                case 'groupmenu':
                    piyo.sendText(from, groupmenu(prefix))
                    break
                case 'downloadmenu':
                    piyo.sendText(from, downloadmenu(prefix))
                    break
                ///////////////////////////////////////////////////MENU////////////////////////////////////////////////////////////
                case 'speed':
                case 'ping':
                    ...
                    const charged = await piyo.getIsPlugged();
                    const device = await piyo.getMe()
                ...
                case 'menu':
                case 'm':
                    const levlmenu = point.getLevelingLevel(sender.id, _point)
                    const xpmenuu = point.getLevelingPoint(sender.id, _point)
                    ...
                    let tod = `${timee}`;
                    await piyo.sendFile(from, pepi, 'piyo.jpg', menud(pushname,role,xpmenuu,premi,levlmenu, _registered, coloo, tod))
                    break
                case 'menuu':
                case 'mn':
                    const levlmenuu = point.getLevelingLevel(sender.id, _point)
                    const xpmenuuu = point.getLevelingPoint(sender.id, _point)
                    const premii = isPremium ? 'Premium' : 'Free'
                    const colooo = await piyo.getAllChatIds()
                    let todd = `${timee}`;
                    if (!isRegistered) return piyo.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Denbot, untuk pendaftaran bisa menggunakan ${prefix}register nama | Jenis Kelamin. Contoh: ${prefix}register ${pushname}|cewe`, id)
                    if (args[0] === '1') {
                    await piyo.sendText(from, systemmenu(prefix))
                } else if (args[0] === '2') {
                    await piyo.sendText(from, mediamenuu(prefix))
                } else if (args[0] === '3') {
                    await piyo.sendText(from, toolmenu(prefix))
                } else if (args[0] === '4') {
                    await piyo.sendText(from, animemenu(prefix))
                } else if (args[0] === '5') {
                    await piyo.sendText(from, menu18(prefix))
                } else if (args[0] === '7') {
                    await piyo.sendText(from, tobatmenu(prefix))
                } else if (args[0] === '8') {
                    await piyo.sendText(from, stickermenu(prefix))
                } else if (args[0] === '9') {
                    await piyo.sendText(from, nulismenu(prefix))
                } else if (args[0] === '10') {
                    await piyo.sendText(from, downloadmenu(prefix))
                } else if (args[0] === '11') {
                    await piyo.sendText(from, imagemenu(prefix))
                } else if (args[0] === '12') {
                    await piyo.sendText(from, logomenu(prefix))
                } else if (args[0] === '13') {
                    await piyo.sendText(from, gamemenu(prefix))
                } else if (args[0] === '14') {
                    await piyo.sendText(from, groupmenu(prefix))
                } else {
                    await piyo.sendFile(from, nadhirasayang, 'piyo.jpg', menuId.textmenubaru(pushname,role,xpmenuuu,premii,levlmenuu, _registered, colooo, todd))
                    }
                    break
                case 'rules':
                case 'rule':
                ...
                case 'donasi':
                    ...
                    break
                case 'ownerbot':
                    return await piyo.sendContact(from, ownerNumber)
                        .then(() => piyo.sendText(from, 'Jika kalian ingin request fitur silahkan chat nomor owner!'))
                    break
                case 'ob':
                   case'owner':
                       const owntolink = '6282144323683'
                   await piyo.sendLinkWithAutoPreview(from, `https://wa.me/${owntolink}`, `Here's my owner number\nhttps://wa.me/${owntolink}`)
                   break
                case 'bal':
                    if (!isRegistered) return piyo.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user DenBot, untuk pendaftaran bisa menggunakan /register nama | Jenis Kelamin. Contoh: /register ${pushname}|cewe`, id)
                ...
                case 'sf':
					if (isMedia && type === 'image') {
						...
						await piyo.sendImageAsSticker(from, imageBase64, StickerMetadata)
						console.log(color(`Sticker processed for ${processTime(t, moment())} seconds`, 'aqua'))
						...
						piyo.reply(from, mess.error.St, id)
					}
					break
                case 'stc':
				case 'sc':
                ...
                case 's':
					...
					if (isMedia && type === 'video' || mimetype == 'sticker/gif') {
						...
						try {
							...
							await piyo.sendMp4AsSticker(from, vidbase, gifxyz, StickerMetadata)
								.then(async () => {
									console.log(color(`Sticker Gif processed for ${processTime(t, moment())} seconds`, 'aqua'))
								})
						} catch (err) {
							console.log(err)
							piyo.reply(from, 'Durasi video terlalu panjang, mohon kecilkan sedikit\nminimal 9 detik', id)
						}
						...
						try {
							...
							await piyo.sendMp4AsSticker(from, videoBase64, gifxyz, StickerMetadata)
								.then(async () => {
									console.log(color(`Sticker Gif processed for ${processTime(t, moment())} seconds`, 'aqua'))
								})
						} catch (err) {
							console.error(err)
							await piyo.reply(from, `Ukuran video terlalu besar\nMaksimal size adalah 1MB!`, id)
						}
					} else {
						await piyo.reply(from, `Ukuran video terlalu besar`, id)
					}
					break
				case 'sgcrop':
                case 'sgc':
					if (isMedia && type === 'video' || mimetype === 'sticker/gif') {
						...
						await piyo.reply(from, `Reply/post video atau gif dengan caption ${prefix}sgif`, id)
					}
					break
                    case 'emojisticker':
                    case 'emojistiker':
                    case 'emoji':
                    case 'emot':
                    if (q.length == 0) return piyo.reply(from, `Kirim perintah ${prefix}emojisticker [emoji]\nContoh : ${prefix}emojisticker 😫`, id)
                    emoji.get(q)
                    .then(emoji => {
                                console.log(color(`Creating sticker emoji => ${emoji.emoji}`, 'cyan'))
                                piyo.sendImageAsSticker(from, emoji.images[0].url, StickerMetadata)
                                    .then(() => {
                    ...
                    case 'nobg': {
                        if (isMedia || isQuotedImage) {
                            ...
                            const mediaData = await decryptMedia(encryptMedia, uaOverride)
                            const imageBase64 = `data:${_mimetype};base64,${mediaData.toString('base64')}`
                            await piyo.sendImageAsSticker(from, imageBase64, StickerNobgcrop)
                        } else {
                            await piyo.reply(from, 'Format pesan salah...', id)
                        }
                    }
                    ...
                    case 'nobgfull': {
                        if (isMedia || isQuotedImage) {
                            ...
                            const mediaData = await decryptMedia(encryptMedia, uaOverride)
                            const imageBase64 = `data:${_mimetype};base64,${mediaData.toString('base64')}`
                            await piyo.sendImageAsSticker(from, imageBase64, StickerNobg )
                        } else {
                            await piyo.reply(from, 'Format pesan salah...', id)
                        }
                    }
                ...
                case 'stikergif':
                    ...
                    if (isMedia && type === 'video' || mimetype === 'image/gif') {
                        ...
                        try {
                            const mediaData = await decryptMedia(message, uaOverride)
                            await piyo.sendMp4AsSticker(from, mediaData, { fps: 24, startTime: `00:00:00.0`, endTime: `00:00:05.0`, loop: 0 },gifcrop, StickerMetadatacrop)
                                .then(async () => {
                                    console.log(color(`Sticker processed for ${processTime(t, moment())} seconds`, 'aqua'))
                                })
                        } catch (err) {
                    ...
                    case 'startgif': //By: Thoriq Azzikra
					if (isMedia && type === 'video' || mimetype === 'sticker/gif') {
						try {
							await piyo.reply(from, ind.wait(), id)
							...
							await piyo.sendMp4AsSticker(from, vidBase64, { crop: true, loop: 0, fps: 30, square: 240, startTime: `00:00:${mulai}.0`, endTime: `00:00:${akhir}.0` }, StickerMetadatacrop)
						} catch (err) {
							console.log(err)
							piyo.reply(from, 'Kecilkan skala video!\nMinimal 240x240', id)
						}
					} else if (quotedMsg && quotedMsg.type === 'sticker' || quotedMsg && quotedMsg.type === 'video') {
						await piyo.reply(from, ind.wait(), id)
						try {
							...
							const mediaData = await decryptMedia(quotedMsg, uaOverride)
							const videoBase64 = `data:${quotedMsg.mimetype};base64,${mediaData.toString('base64')}`
							await piyo.sendMp4AsSticker(from, videoBase64, { crop: true, loop: 0, square: 240, fps: 30, startTime: `00:00:${mulaini}.0`, endTime: `00:00:${akhirni}` }, StickerMetadatacrop)
								.then(async () => {
									console.log(color(`Sticker Gif processed for ${processTime(t, moment())} seconds`, 'aqua'))
								})
						} catch (err) {
                ...
                if (quotedMsg && quotedMsg.type === 'audio' || quotedMsg && quotedMsg.type === 'ptt') {
                    ...
                    piyo.reply(from, `vn dengan nama ${nuhi} berhasil disimpan didalam database!`, id)
                } else if (isMedia && type === 'audio' || isMedia && type === 'ptt') {
                    var mediaData = await decryptMedia(message, uaOverride)
                    var filename = `./media/audio/${nuhi}.mp3`
                    await fs.writeFileSync(filename, mediaData)
                    await piyo.reply(from, `vn dengan nama ${nuhi} berhasil disimpan didalam database!`, id)
                } else {
                    return piyo.reply(from, `Error! silahkan coba kembali...`, id)
                }
                listvn.push(nuhi)
                fs.writeFileSync('./lib/database/listvn.json', JSON.stringify(listvn))
                break
                case 'delallvn':
                    if (!isOwnerB) return piyo.reply(from, 'Fitur ini khusus Owner Bot', id)
                let dellall = listvn.includes(chats)
                listvn.splice(dellall)
                fs.writeFileSync('./lib/database/listvn.json', JSON.stringify(listvn))
                piyo.reply(from, `semua vn didalam database berhasil dihapus`, id)
                break
                case 'delvn':
                ...
                piyo.reply(from, 'vn berhasil didelete dari database', id)
                    .catch(err => {
                        console.log(err)
                ...
                break
                case 'delallimg':
                    if (!isOwnerB) return piyo.reply(from, `Fitur ini hanya bisa digunakan oleh owner bot!`, id)
                ...
                piyo.reply(from, 'semua image didalam database berhasil dihapus', id)
                    .catch(err => {
                        piyo.reply(from, err.message, id)
                    })
                break
                case 'addvid':
                    if (!isOwnerB) return piyo.reply(from, 'Owner Bot only', id)
                if (quotedMsg && quotedMsg.type == 'video') {
                    var mediaData = await decryptMedia(quotedMsg, uaOverride)
                    var filevid = `./media/video/${q}.mp4`
                    fs.writeFileSync(filevid, mediaData)
                    piyo.reply(from, `video dengan nama ${q} berhasil dimasukkan kedalam database`, id)
                    ...
                    piyo.reply(from, `post/reply video dengan caption`, id)
                }
                listvid.push(q)
                ...
                piyo.reply(from, `video dengan nama ${q} berhasil didelete dari database`, id)
                    .catch(err => {
                        console.log(err)
                        piyo.reply(from, err.message, id)
                    })
                ...
                kumtul += '╚═〘 *DEN  B O T* 〙'
                await piyo.reply(from, arrayvid, id)
                break
                case 'addimg':
                    case 'addimage':
                    let addmg = q
                if (quotedMsg && quotedMsg.type === 'image') {
                    ...
                    await fs.writeFile(filename, mediaData)
                    await piyo.reply(from, `image dengan nama ${addmg} berhasil disimpan didalam database!`, id)
                    ...
                    return piyo.reply(from, `Error!, silahkan coba kembali nanti...`, id)
                }
                listimg.push(addmg)
                fs.writeFileSync('./lib/database/listimage.json', JSON.stringify(listimg))
                break
                ...
                piyo.reply(from, `image dengan nama ${delx} berhasil didelete dari database`, id)
                    .catch(err => {
                        console.log(err)
                        piyo.reply(from, err.message, id)
                    })
                ...
                case 'fakethumb':
                    if (isMedia && isImage || isQuotedImage) {
                        ...
                        if (!strs) return piyo.reply(from, 'Masukkan param https/http', id)
                        await piyo.reply(from, ind.wait(), id)
                        ...
                        await piyo.sendLinkWithAutoPreview(from, bodslice, '', imageBse)
                    } else {
                        piyo.reply(from, `Reply foto dengan caption ${prefix}fakethumb url`, id)
                    }
                break
                case 'readmore':
                ...
                case 'water':
                    ...
                    break
                case 'qrcode':
                if (!q) return piyo.reply(from, `Ketik .qrcode teksnya`, id)
                await piyo.sendFileFromUrl(from, `https://api.akuari.my.id/other/qrcode?text=${q}`)
                break
                case 'qrcodes':
                if (!q) return piyo.reply(from, `Ketik .qrcode teksnya`, id)
                await piyo.sendStickerfromUrl(from, `https://api.akuari.my.id/other/qrcode?text=${q}`)
                break
                case 'ttg':
                ...
                case 'tlight':
                    ...
                    const yo5 = await axios.get(`https://api.zeks.me/api/tlight?text=${q}&apikey=administrator`)
                ...
                case '3d':
                    ...
                    const yo6 = await axios.get(`https://api.zeks.me/api/text3dbox?apikey=administrator&text=${q}`)
                ...
                case 'pubg':
                    ...
                    const pabji = await axios.get(`https://api.zeks.me/api/pubglogo?text1=${pubg1}&text2=${pubg2}&apikey=administrator`)
                ...
                case 'csgo':
                    ...
                    const csgo = await axios.get(`https://api.zeks.me/api/cslogo?text=${q}&apikey=administrator`)
                ...
                case 'blackpink':
                    ...
                    await piyo.sendFileFromUrl(from, `https://api.zeks.me/api/logobp?text=${blpk}&apikey=${apizeks}`, 'blackpink.jpg', '', id)
                    break
                case 'pornhub':
                    if (args.length == 2) return piyo.reply(from, `Kirim perintah */pornhub [ |Teks1|Teks2 ]*, contoh */pornhub |Alvio|DenBot*`, id)
                    ...
                    if (argz.length >= 2) {
                        ...
                        piyo.sendFileFromUrl(from, `https://api.zeks.me/api/phlogo?text1=${lpornhub}&text2=${lpornhub2}&apikey=${apizeks}`)
                    } else {
                ...
                case 'ffmaker':
                    ...
                    await piyo.sendFileFromUrl(from, `https://api.zeks.me/api/epep?text=${q}&apikey=${apizeks}`, 'error.jpg', '', id)
                ...
                case 'wolf2':
                    ...
                    if (argz.length === 3) {
                        ...
                        const wolf12 = argz[1]
                        const wolf22 = argz[2]
                        piyo.sendFileFromUrl(from, `https://api.zeks.me/api/wolflogo?apikey=${apizeks}&text1=${wolf12}&text2=${wolf22}`, 'wolf2.jpg', 'neh bang', id)
                    } else {
                ...
                case 'wolf':
                    ...
                    if (argz.length === 3) {
                        ...
                        const wolf1 = argz[1]
                        const wolf2 = argz[2]
                        piyo.sendFileFromUrl(from, `https://api.zeks.me/api/wolflogo?apikey=${apizeks}&text1=${wolf1}&text2=${wolf2}`, 'wolf2.jpg', 'neh bang', id)
                    } else {
                        await piyo.reply(from, `Wrong Formt!\n[❗] Kirim perintah *${prefix}wolf2 [ |Teks1|Teks2 ]*, contoh *${prefix}wolf2 |piyo|bot*`, id)
                    }
                    break
                ...
                case 'reminder': // by Slavyan
                    if (!q.includes('|')) return await piyo.reply(from, ind.reminfor(), id)
                ...
                case 'suit':
                    ...
                    break
                case 'setpic':
                    if (!isOwnerBot) return piyo.reply(from, `Perintah ini hanya bisa di gunakan oleh Owner Bot!`, id)
                if (isMedia) {
                    const mediaData = await decryptMedia(message)
                    const imageBase64 = `data:${mimetype};base64,${mediaData.toString('base64')}`
                    await piyo.setProfilePic(imageBase64)
                    piyo.sendTextWithMentions(from, `Makasih @${serial} Foto Profilenye..`)
                } else if (quotedMsg && quotedMsg.type == 'image') {
                    const mediaData = await decryptMedia(quotedMsg)
                    const imageBase64 = `data:${quotedMsg.mimetype};base64,${mediaData.toString('base64')}`
                    await piyo.setProfilePic(imageBase64)
                    piyo.sendTextWithMentions(from, `Makasih @${serial} Foto Profilenya 😘`, id)
                } else {
                    piyo.reply(from, `Wrong Format!\n⚠️ Harap Kirim Gambar Dengan ${prefix}setpic`, id)
                }
                break
                case 'getpic':
                ...
                case 'caklontong':
                    if (!isGroupMsg) return piyo.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
                    try {
                        const resp = await axios.get('https://api.wibusoft.com/api/permainan/cak-lontong')
                        if (resp.data.error) return piyo.reply(from, resp.data.error, id)
                    ...
                    if (!isGroupMsg) return piyo.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
                    try {
                        const resp = await axios.get('https://api.wibusoft.com/api/permainan/tebak-lagu')
                        if (resp.data.error) return piyo.reply(from, resp.data.error, id)
                        piyo.sendFileFromUrl(from, resp.data.result.soal, 'tebaklagu.mp3', '_Silahkan Jawab Maksud Dari Gambar Ini_\nHanya Satu Jawaban Untuk Satu Orang\nJawab Semua Dengan Huruf kecil\nContoh = anak maling\nDiberi waktu 30detik Untuk Menjawab Kuiz', id)
                        piyo.sendText(from, `30 Detik Lagi...`, id)
                        await rugaapi.sleep(10000)
                        piyo.sendText(from, `20 Detik Lagi...`, id)
                        await rugaapi.sleep(10000)
                        piyo.sendText(from, `10 Detik Lagi...`, id)
                        await rugaapi.sleep(10000)
                        const jwbaan = `➸ Jawaban : ${resp.data.result.jawaban}\n➸ Deskripsi : ${resp.data.result.deskripsi}`
                        await piyo.sendText(from, jwbaan, id)
                    } catch (err) {
                        console.error(err.message)
                        await piyo.sendFileFromUrl(from, errorurl2, 'error.png', '💔️ Maaf, Soal Quiz tidak ditemukan')
                        piyo.sendText(ownerNumber, 'Zodiak Error : ' + err)
                    ...
                    if (!isGroupMsg) return piyo.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
                    try {
                        const resp = await axios.get('https://api.wibusoft.com/api/permainan/susun-kata')
                        if (resp.data.error) return piyo.reply(from, resp.data.error, id)
                        const anm22 = `➸ Soal : ${resp.data.result.soal}\n➸ Kalau sudah tau jawaban silahkan ketik dibawah dan tunggu waktu habis`
                        piyo.reply(from, anm22, id)
                        piyo.sendText(from, `30 Detik Lagi...`, id)
                        await rugaapi.sleep(10000)
                        piyo.sendText(from, `20 Detik Lagi...`, id)
                        await rugaapi.sleep(10000)
                        piyo.sendText(from, `10 Detik Lagi...`, id)
                        await rugaapi.sleep(10000)
                        const jwbann = `➸ Jawaban : ${resp.data.result.jawaban}\n➸ Deskripsi : ${resp.data.result.deskripsi}`
                        await piyo.sendText(from, jwbann, id)
                    } catch (err) {
                        console.error(err.message)
                        await piyo.sendFileFromUrl(from, errorurl2, 'error.png', '💔️ Maaf, Soal Quiz tidak ditemukan')
                        piyo.sendText(ownerNumber, 'Zodiak Error : ' + err)
                ...
                case 'tebakgambar': //DenBot
                    if (isKuis) return piyo.reply(from, `Kuis Sedang Berlangsung`, id)
                    if (!isGroupMsg) return piyo.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
                    try {
                        const resp = await axios.get('https://api.zeks.me/api/tebakgambar?apikey=LS8rUDhkEBvP8pELirfBxba01I9')
                        if (resp.data.error) return piyo.reply(from, resp.data.error, id)
                        piyo.sendFileFromUrl(from, resp.data.result.soal, 'tebakgambar.jpg', '_Silahkan Jawab Maksud Dari Gambar Ini_\nHanya Satu Jawaban Untuk Satu Orang\nJawab Semua Dengan Huruf kecil\nContoh = anak maling\nDiberi waktu 30detik Untuk Menjawab Kuiz', id)
                ...
                case 'nextkuiz':
                    ...
                    try {
                        ...
                        const respek = await axios.get('https://api.zeks.me/api/tebakgambar?apikey=LS8rUDhkEBvP8pELirfBxba01I9')
                        if (respek.data.error) return piyo.reply(from, respek.data.error, id)
                        piyo.sendFileFromUrl(from, respek.data.result.soal, 'tebakgambar.jpg', '_Silahkan Jawab Maksud Dari Gambar Ini_\nHanya Satu Jawaban Untuk Satu Orang\nJawab Semua Dengan Huruf kecil\nContoh anak maling ', id)
                    ...
                    if (isOwnerBot) {
                        await piyo.joinGroupViaLink(linkgrup)
                            .then(async () => {
                                ...
                                await piyo.sendText(chekgrup.id, `Halo Anjg~, Im DenBot. Untuk Memulai Bot silahkan ketik  ${prefix}menu`)
                            })
                    } else {
                ...
                case 'next':
                    ...
                    break
                case 'send':
                    if (!isRegistered) return piyo.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user DenBot, untuk pendaftaran bisa menggunakan *.register* nama | no hp.    Contoh: /register |${pushname}|cewe/cowo`, id)
                        ...
                        fetch('https://trace.moe/api/search', {
                            ...
                            .then(resolt => {
                                if (resolt.docs && resolt.docs.length <= 0) {
                                    piyo.reply(from, 'Maaf, saya tidak tau ini anime apa, pastikan gambar yang akan di Search tidak Buram/Kepotong', id)
                                }
                                const {
                                    is_adult,
                                    title,
                                ...
                                piyo.sendFileFromUrl(from, video, 'anime.mp4', teks, id).catch(() => {
                                    piyo.reply(from, teks, id)
                                })
                            })
                            .catch(() => {
                                piyo.reply(from, 'Ada yang Error!', id)
                ...
                piyo.sendText(from, 'Restart Bot in')
                await sleep(1000)
                piyo.sendText(from, '5')
                await sleep(1000)
                piyo.sendText(from, '4')
                await sleep(1000)
                piyo.sendText(from, '3')
                await sleep(1000)
                piyo.sendText(from, '2')
                await sleep(1000)
                piyo.sendText(from, '1')
                await sleep(1000)
                piyo.sendText(from, `Refresh Bot`)
                await sleep(10000)
                piyo.refresh()
                    .then(() => {
                        piyo.reply(from, 'Success refresh page', id)
                    })
                break
                case 'shutdown':
                piyo.sendText(from, 'Shutdown Bot in')
                await sleep(1000)
                piyo.sendText(from, '5')
                await sleep(1000)
                piyo.sendText(from, '4')
                ...
                piyo.sendText(from, '1')
                await sleep(1000)
                piyo.sendText(from, `See y'all later`)
                await sleep(10000)
                piyo.kill(true)
                break
                case 'viewonce':
                    if (args.length == 0) return piyo.reply(from, 'pilih enable atau disable', id)
                if (isGroupMsg) {
                    if (!isGroupAdmins) return piyo.reply(from, 'Command ini hanya bisa digunakan oleh Admin Grup', id)
                    if (args[0] == 'enable') {
                        if (isViewOnceOn) return piyo.reply(from, 'Sudah aktif sebelumnya', id)
                        viewonce.push(chatId)
                        fs.writeFileSync('./lib/database/viewonce.json', JSON.stringify(viewonce))
                        piyo.reply(from, 'Berhasil menyalakan anti view once', id)
                    } else if (args[0] == 'disable') {
                        let teruy = viewonce.indexOf(chatId)
                        viewonce.splice(teruy, 1)
                        fs.writeFileSync('./lib/database/viewonce.json', JSON.stringify(viewonce))
                        piyo.reply(from, 'Berhasil menonaktifkan anti view once', id)
                    }
                    ...
                    piyo.reply(from, 'Pilih enable atau disable', id)
                }
                break
                case 'private':
                    if (!isOwnerBot) return piyo.reply(from, 'Perintah ini hanya bisa digunakan oleh owner Bot!', id)
                if (setting.banChats === true) return
                setting.banChats = true
                banChats = true
                fs.writeFileSync('./lib/database/setting.json', JSON.stringify(setting, null, 2))
                piyo.reply(from, 'Private Commands has been enable', id)
                break
                case 'public':
                    if (!isOwnerBot) return piyo.reply(from, 'Perintah ini hanya bisa digunakan oleh owner Bot!', id)
                ...
                banChats = false
                fs.writeFileSync('./lib/database/setting.json', JSON.stringify(setting, null, 2))
                piyo.reply(from, 'Public commands has been enable', id)
                break
                case 'charsgenshin': {
                ...
                case 'ppcouple':
                    piyo.reply(from, ind.wait(), id);
                    axios.get('https://api-xcoders.xyz/api/random/ppcouple?apikey=tixL5bhvWW').then(res => {
                        piyo.sendFileFromUrl(from, res.data.result.ppcwo, 'ppcwo.jpeg', 'COWOK :>', id);
                        piyo.sendFileFromUrl(from, res.data.result.ppcwe, 'ppcwe.jpeg', 'CEWEK :>', id);
                    });
                    break
                case 'wallpaperanime':
                    piyo.reply(from, ind.wait(), id);
                ...
                //nsfw menu
                case 'nsfww':
                    if (!q) return await piyo.reply(from, ind.nsfww(), id)
                    if (banChat()) return await piyo.reply(from, `Stay Halal Brother`, id)
                    await piyo.reply(from, ind.wait(), id)
                    piyo.sendFileFromUrl(from, `https://hardianto.xyz/api/anime/random?nsfw=${q}&apikey=hardianto`, 'nsfw.jpg', 'CIH', id)
                    break
                case 'nsfww3':
                    if (banChat()) return await piyo.reply(from, `Stay Halal Brother`, id)
                    await piyo.reply(from, ind.wait(), id)
						piyo.sendStickerfromUrl(from, `https://api.dapuhy.ga/api/nsfw/nsfwhentaigif?apikey=denisetiawan`, '', 'bakaaa hentaii>~<',id);
						piyo.sendFileFromUrl(from, `https://api.dapuhy.ga/api/nsfw/nsfwhentaigif?apikey=denisetiawan`, StickerMetadata)
                    case 'classic':
					if (banChat()) return await piyo.reply(from, `Stay Halal Brother`, id)
                    await piyo.reply(from, ind.wait(), id)
					axios.get('https://hardianto.xyz/api/anime/random?nsfw=randomHentaiGif&apikey=hardianto').then(result => {
						piyo.sendStickerfromUrl(from, result.data, StickerMetadata)
					})
					break
					break
                    case 'nsfww2':
                    if (!q) return await piyo.reply(from, ind.nsfww2(), id)
                    if (banChat()) return await piyo.reply(from, `Stay Halal Brother`, id)
                    await piyo.reply(from, ind.wait(), id)
                    piyo.sendFileFromUrl(from, `https://revita.herokuapp.com/api/nsfw/${q}?apikey=ApiRevita `, 'nsfww.jpg', 'CIH', id)
                    break
                ...
                case 'gnek':
                    ...
                    await piyo.reply(from, ind.wait(), id);
                    axios.get('https://hardianto.xyz/api/anime/random?nsfw=randomHentaiGif&apikey=hardianto').then(res => {
                        piyo.sendFileFromUrl(from, res.data, 'bakaaaaa baka hentai');
                    });
                    break
                case 'gifhentai':
                    if (banChat()) return await piyo.reply(from, `Stay Halal Brother`, id)
                ...
                case 'loli':
                    await piyo.reply(from, ind.wait(), id)
                    piyo.sendFileFromUrl(from, `https://api-xcoders.xyz/api/anime/loli?apikey=tixL5bhvWW`, 'loli.jpeg', '*LOLI*', id)
                    break
                ...
                getnekos.nsfw.boobs()
                    .then(boobs => {
                        piyo.sendFileFromUrl(from, boobs.url, '', '', id)
                        piyo.sendImageAsSticker(from, boobs.url, StickerMetadata)
                    })
                break
                /////////////////////////////////////////////////////////MENU EDUKASI////////////////////////////////////////////////////////////
                case 'kbbi':
                ...
                case 'nulishd':
                    ...
                    await piyo.sendFileFromUrl(from, `https://api.zeks.me/api/nulis?text=${q}&apikey=${apizeks}`, 'error.jpg', 'Kalo filenya berbentuk dokumen berarti eror', id)
                    break
                case 'nuliskanan': {
                ...
                case 'jadwalsholat':
                    ...
                    const jadwalShalat = await axios.get(`https://api-xcoders.xyz/api/muslim/jadwalshalat?query=${daerah}&apikey=tixL5bhvWW`)
                    if (jadwalShalat.data.error) return piyo.reply(from, jadwalShalat.data.error, id)
                    const { imsak, dzuhur, ashar, maghrib, isya, lokasi } = await jadwalShalat.data
                    arrbulan = ["Januari", "Februari", "Maret", "April", "Mei", "Juni", "Juli", "Agustus", "September", "Oktober", "November", "Desember"];
                    var date = new Date();
                    tgl = new Date().getDate()
                    bln = new Date().getMonth()
                    thn = new Date().getFullYear()
                    const resultJadwal = `「 JADWAL SHALAT 」\n\nJadwal shalat di ${lokasi}, ${tgl}-${arrbulan[bln]}-${thn}\n\nSubuh : ${imsak}\nDzuhur : ${dzuhur}\nAshar : ${ashar}\nMaghrib : ${maghrib}\nIsya : ${isya}`
                    await limitAdd(serial)
                    break
                /////////////////////////////////////
                case 'jadwalbola':
                ...
                case 'tiktoknowm':
                    await piyo.reply(from, ind.wait(), id)
                    const tp = await axios.get(`https://api.wibusoft.com/api/tiktok/nowm?url=${q}`)
                ...
                case 'tiktok':
                    await piyo.reply(from, ind.wait(), id)
                    await piyo.sendFileFromUrl(from, `http://piyobot.cf/download.png`, 'image.jpg', `Video Ditemukan...\n\nDenBot\n\n*_Sabar, Piyo lagi ngirim Videonya_*`, id)
                    await piyo.sendFileFromUrl(from, `https://api.wibusoft.com/api/tiktok/wm?url=${q}`, 'piyo.mp4', `*Tiktok Downloader*`, id)
                    break

                case 'tiktokmusic':
                    if (!q) return piyo.reply(from, `Silahkan ketik /tiktokmusic link tiktoknya`, id)
                    await piyo.reply(from, ind.wait(), id)
                    await piyo.sendFileFromUrl(from, `https://api.wibusoft.com/api/tiktok/audio?url=${q}`, 'tiktok.mp3', '', id)
                    break
                ...
                case 'searchspotify':
                    ...
                    break
                    case 'joox3': //PAKE API VIDE FIKRI BIAR ADA SIZE LAGUNYA
                    if (!q) return piyo.reply(from, `Silahkan  ketik ${prefix}play judulnya`, id)
                    await piyo.reply(from, 'Wait..', id)
                    const sass = await axios.get(`https://docs-jojo.herokuapp.com/api/joox?q=${q}`)
                    await piyo.sendFileFromUrl(from, sass.data.imgSrc, 'gambar.jpg', `Title: ${sass.data.msong}\n\n───────────⚪───────────\n(っ◔◡◔)っ   ◄◄⠀▐▐ ⠀►► ───○ 🔊\n───────────⚪───────────\n➥Size: ${sass.data.msize}\n➥Type: Mp3 \n*Mohon Tunggu Bot Akan Mengirim Audio!*`, id)
                    const ddd = await fetch(sass.data.audio)
                    const bufffff = await ddd.buffer()
                    await fs.writeFile('./media/play.mp3', bufffff)
                    await piyo.sendFile(from, './media/play.mp3', 'nih kak', '', id)
                    await fs.unlinkSync('./media/play.mp3')
                    break
                    case 'joox2': //PAKE API VIDE FIKRI BIAR ADA SIZE LAGUNYA
                    if (!q) return piyo.reply(from, `Silahkan  ketik ${prefix}play judulnya`, id)
                    await piyo.reply(from, 'Wait..', id)
                    const ssss = await axios.get(`https://api.zeks.me/api/joox?apikey=${apizeks}&q=${q}`)
                    await piyo.sendFileFromUrl(from, ssss.data.data[0].thumb, 'gambar.jpg', `Title: ${ssss.data.data[0].judul}\n\n───────────⚪───────────\n(っ◔◡◔)っ   ◄◄⠀▐▐ ⠀►► ───○ 🔊\n───────────⚪───────────\n➥Size: ${ssss.data.data[0].size}\n➥Type: Mp3 \n*Mohon Tunggu Bot Akan Mengirim Audio!*`, id)
                    const dd = await fetch(ssss.data.data[0].audio)
                    const buffff = await dd.buffer()
                    await fs.writeFile('./media/play.mp3', buffff)
                    await piyo.sendFile(from, './media/play.mp3', 'nih kak', '', id)
                    await fs.unlinkSync('./media/play.mp3')
                    break
                case 'play': //PAKE API VIDE FIKRI BIAR ADA SIZE LAGUNYA
                    if (!q) return piyo.reply(from, `Silahkan  ketik ${prefix}play judulnya`, id)
                    await piyo.reply(from, 'Wait..', id)
                    const getvids = await axios.get(`https://api.zeks.me/api/ytplaymp3?apikey=${apizeks}&q=${q}`)
                ...
                case 'playv':
                    if (!q) return piyo.reply(from, `Silahkan  ketik ${prefix}playv judulnya`, id)
                    if (!isPremium) return piyo.reply(from, `Maaf, ini adalah fitur premium, untuk menggunakan fitur ini silahkan beli premium ke owner \nUntuk Harga\n\n10k Perbulan\n5k Perpanjang`, id)
                    piyo.reply(from, 'Wait.. Sedang Di Proses!', id)
                    const getvid = await axios.get(`https://api.zeks.me/api/ytplaymp4?q=${q}&apikey=${apizeks}`)
                ...
                case 'playvn':
                    if (!q) return piyo.reply(from, `Silahkan  ketik ${prefix}playvn judulnya`, id)
                    await piyo.reply(from, `Wait`, id)
                    const plan = await axios.get(`https://api.zeks.me/api/ytplaymp3?apikey=${apizeks}&q=${q}`)
                ...
                case 'ytmp3':
                    await piyo.reply(from, ind.wait(), id)
                    try {
                        const yte = await axios.get(`https://revita.herokuapp.com/api/download/ytmp3?url=${q}&apikey=ApiRevita`)
                        await piyo.sendFileFromUrl(from, yte.data.result.thumb, 'piyo.jpg', `*「 YOUTUBE MP3 」*\n\n• *Judul* : ${yte.data.result.title}`, id)
                        const bufmp3 = await fetch(yte.data.result.link.url)
                ...
                case 'ttsong':
                    await piyo.reply(from, ind.wait(), id)
                    try {
                        const ttss = await axios.get(`https://api.wibusoft.com/api/tiktok/audio?url=${q}`)

                        const buffmp3 = await fetch(ttss.data.result.url)
                        const mp3fbuf = await buffmp3.buffer()
                        await fs.writeFile(`./media/${sender.id}.mp3`, mp3fbuf)
                        await piyo.reply(from, `Apakah mau dikirim file berbentuk vn atau mp3 ?\nKalo vn ketik iya kalo mp3 ketik ga`, id)
                        usermp3.push(sender.id)
                        fs.writeFileSync('./settings/usermp3.json', JSON.stringify(usermp3))
                    } catch (err) {
            ...
            case 'ttm':
                    await piyo.reply(from, ind.wait(), id)
                    try {
                        const ttm = await axios.get(`https://api.wibusoft.com/api/tiktok/nowm?url=${q}`)
                        const buufmp3 = await fetch(ttm.data.result.url)
                        const mp3bbuf = await buufmp3.buffer()
                        await fs.writeFile(`./media/${sender.id}.mp3`, mp3bbuf)
                        await piyo.sendPtt(from, `./media/${sender.id}.mp3`, 'nih kak', '', id)
            fs.unlinkSync(`./media/${sender.id}.mp3`)
                    } catch (err) {
                        console.error(err.message)
                        await piyo.sendFileFromUrl(from, errorurl2, 'error.png', '💔️ Maaf, Ada Sedikit Error')
                    ...
                    await piyo.reply(from, ind.wait(), id)
                    try {
                        const ttnw = await axios.get(`https://api.wibusoft.com/api/tiktok/nowm?url=${q}`)
                        const bufmp4 = await fetch(ttnw.data.result.url)
                    ...
                    await piyo.reply(from, ind.wait(), id)
                    try {
                        const ttwm = await axios.get(`https://api.wibusoft.com/api/tiktok/wm?url=${q}`)
                        const bufmp4 = await fetch(ttwm.data.result.url)
                        const mp4buf = await bufmp4.buffer()
                        await fs.writeFile(`./media/${sender.id}mp4`, mp4buf)
                        await piyo.sendFile(from, `./media/${sender.id}mp4`, 'piyo.mp4', 'Nih Kak', id)
                    } catch (err) {
                ...
                case 'ytmp4':
                    await piyo.reply(from, ind.wait(), id)
                    try {
                        const yt = await axios.get(`https://revita.herokuapp.com/api/download/ytmp4?url=${q}&apikey=ApiRevita`)
                        await piyo.sendFileFromUrl(from, yt.data.result.thumb, 'piyo.jpg', `*「 YOUTUBE MP4 」*\n\n• *Judul* : ${yt.data.result.title}\n\n• *Source* : ${q}\n\nSilahkan tunggu file media sedang dikirim mungkin butuh beberapa menit_`, id)
                        const bufmp4 = await fetch(yt.data.result.url)
                        const mp4buf = await bufmp4.buffer()
                        await fs.writeFile(`./media/${sender.id}mp4`, mp4buf)
                        await piyo.sendFile(from, `./media/${sender.id}mp4`, 'piyo.mp4', 'Nih Kak', id)
                    } catch (err) {
                    ...
                    break
                    case 'igstalkk':
                    if (args.length == 0) piyo.reply(from, `Kirim perintah ${prefix}igstalk username\nContoh ${prefix}igstalk duar_amjay`, id)
                    await piyo.reply(from, ind.wait(), id)
                    scrape.igstalkk(q)
                        .then(result => {
                            console.log(result)
                            piyo.sendFileFromUrl(from, result.thumbnail, '', `*- Username:* ${result.username}\n*- Fullname:* ${result.fullname}\n*- Followers:* ${result.followers}\n*- Following:* ${result.following}\n*- Verified:* ${result.verified}\n*- Bio:* ${result.bio}`, id)
                                .catch(err => {
                                    console.log(err)
                                    piyo.reply(from, 'Terjadi kesalahan, silahkan ulangi', id)
                                })
            ...
            case 'igstalk2':
                    await piyo.reply(from, ind.wait(), id)
                    if (arghh.length === 1) return piyo.reply(from, `Kirim perintah ${prefix}igstalk username\nContoh ${prefix}igstalk duar_amjay`, id)
                    argz = body.trim().split(' ')
                    console.log(...argz[1])
                    var slicedArgs = Array.prototype.slice.call(argz, 1);
                    console.log(slicedArgs)
                    const istalkk = await slicedArgs.join(' ')
                    console.log(istalkk)
                    try {
                        const istalkk2 = await axios.get(`https://hardianto.xyz/api/igstalk?username=${q}&apikey=hardianto`)
                        const { bio, followers, follow, fullname, username, thumbnail, private } = istalkk2.data
                        const istalkk3 = `User Ditemukan!
➸ *USERNAME:* ${username}
➸ *Nama:* ${fullname}
➸ *Bio:* ${bio}
➸ *Mengikuti:* ${follow}
➸ *Pengikut:* ${followers}`
                        const pictk = await bent("buffer")(thumbnail)
                        const base64 = `data:image/jpg;base64,${pictk.toString("base64")}`
                        piyo.sendImage(from, base64, username, istalkk3, id)
                    } catch (err) {
                        console.error(err.message)
                ...
                case 'igstalk':
                    ...
                    console.log(istalk)
                    try {
                        const istalk2 = await axios.get(`https://api.wibusoft.com/api/instagram/stalk?user=${q}`)
                        const { bio, follower_count, following_count, name, username, profile_url, private } = istalk2.data.result
...
➸ Pengikut: ${follower_count}`
                        const pictk = await bent("buffer")(profile_url)
                        const base64 = `data:image/jpg;base64,${pictk.toString("base64")}`
                ...
                case 'tiktokstalk':
                    piyo.reply(from, ind.wait(), id)
                    if (isLimit(serial)) return piyo.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
                    if (arghh.length === 1) return piyo.reply(from, `Kirim perintah *${prefix}tiktokstalk @username*\nContoh *${prefix}tiktokstalk @duar_amjay*`, id)
                    argz = body.trim().split(' ')
                    console.log(...argz[1])
                    var slicedArgs = Array.prototype.slice.call(argz, 1);
                    console.log(slicedArgs)
...
➸ *Jumlah Postingan:* ${video}`
                ...
                case 'xnxx': {
                    ...
                    pemisah = xtext.split("|")
                    const search = pemisah[0]
                    const responlo = await requests("https://caliph71.xyz/api/xnxx/search?apikey=VdbOJya2&query=" + xtext)
                    const datas = await responlo.json()
                ...
                case 'bokep': {
                    ...
                    const ob = await axios.get(`https://caliph71.xyz/api/xnxx/detail?apikey=VdbOJya2&url=${q}`)
                    const vvv = ob.data
                    await piyo.reply(from, `Mohon tunggu , Video terikirim akan lebih lama jika size besar`, id)
                    await piyo.sendFileFromUrl(from, vvv.result.url, 'bokep.mp4', `Title : ${viv.result.title}`, id)
                }
                    break

                case 'xnxxdl': {
                    if (!isPremium) return piyo.reply(from, `Khusus Premium `, id)
                    const vioo = await axios.get(`https://caliph71.xyz/api/xnxx/detail?apikey=VdbOJya2&url=${q}`)
                    const viv = vioo.data
                    await piyo.reply(from, `Mohon tunggu , Video terikirim akan lebih lama jika size besar`, id)
                    await piyo.sendFileFromUrl(from, viv.result.files.low, 'bokep.mp4', `Title : ${viv.result.title}`, id)
                }
                    break

                case 'xvidl': {
                    if (!isPremium) return piyo.reply(from, `Khusus Premium `, id)
                    const vio = await axios.get(`https://api-xcoders.xyz/api/download/xvideos?url=${q}&apikey=tixL5bhvWW`)
                ...
                case 'kisahnb':
                    await piyo.reply(from, ind.wait(), id)
                    const knb = await axios.get(`https://api-xcoders.xyz/api/muslim/kisahnabi?nabi=${q}&apikey=tixL5bhvWW`)
                    const likk = knb.data.result
                    await piyo.reply(from,`*KISAH NABI*:\n *NAMA : ${likk.name}*\n *LAHIR : ${likk.kelahiran}*\n *WAFAT USIA : ${likk.wafat_usia}*\n *KOTA : ${likk.singgah}*\n *KISAH : * ${likk.kisah}`, id)
                    break
                case 'lirik':
                    await piyo.reply(from, ind.wait(), id)
                    const lirikk = await axios.get(`https://hardianto.xyz/api/info/lirik?query=${q}&apikey=hardianto`)
                    const lik = lirikk.data
                    await piyo.sendFileFromUrl(from, lirikk.data.thumb, 'gambar.jpg',`*LIRIK*:\n${lik.lirik}`, id)
                    break
                case 'ptl':
                    if (!isPremium) return piyo.reply(from, `Maaf, ini adalah fitur premium, untuk menggunakan fitur ini silahkan beli premium ke owner \nUntuk Harga\n\n 10k Perbulan\n5k Perpanjang`, id)
                ...
                case 'darkjokes':
                    const darkjokex = await axios.get('https://api.zeks.me/api/darkjokes?apikey=${apizeks}')
                ...
                case 'xnvid':
                    await piyo.reply(from, ind.wait(), id)
                    const xn = await axios.get(`https://caliph71.xyz/api/xnxx/search?apikey=VdbOJya2&query=${q}`)
                    const xnx = xn.data 
                    let xvid = `*「 XNXX SEARCH 」*\n\n*Hasil Pencarian:* ${q}\n` 
                            for (let i = 0; i < xnx.result.length; i++) {
                                xvid += `\n\n*JUDUL:* : ${xnx.result[i].title}\n*INFO:* : ${xnx.result[i].info}\n*URL*: ${xnx.result[i].link}\n=_=_=_=_=_=_=_=_=_=_=_=_=`
                            }
                            await piyo.reply(from, xvid, id);
                ...
                case 'shopee2':
                    if (!q) return await piyo.reply(from, ind.wrongFormat(), id)
                    await piyo.reply(from, ind.wait(), id)
                    const sho = await axios.get(`https://api.zeks.me/api/shopee?apikey=LS8rUDhkEBvP8pELirfBxba01I9&q=${q}`)
                    const shop = sho.data 
                    for (let i = 0; i < shop.data.length; i++) {
                    const { name, harga, terjual, location, desc, url, cover } = shop.data[i]
                    piyo.sendFileFromUrl(from, cover, `shope.jpg`, ind.shopee2(name, harga, terjual, location, desc, url))
                                }
                    break
                case 'shopee':
                    if (!q) return await piyo.reply(from, ind.wrongFormat(), id)
                    ...
                    try {
                        rugaapi.shopee(namaBarang, jumlahBarang)
                            .then(async ({ result }) => {
                                for (let i = 0; i < 1; i++) {
                                    const { nama, harga, terjual, location, desc, url, cover } = result.data[i]
                                    await piyo.sendFileFromUrl(from, image_cover, `${nama}.jpg`, ind.shopee(nama, harga, terjual, location, desc, url))
                                }
                ...
                case 'earrape': 
                    if (isMedia && isAudio || isQuotedAudio || isVoice || isQuotedVoice) {
                        ...
                        const bass = await stream2Buffer(write => {
                            ffmpeg(buffer2Stream(mediaData))
                                .audioFilter('acrusher=level_in=2:level_out=6:bits=8:mode=log:aa=1,lowpass=f=3500')
                                .format('mp3')
                                .on('start', commandLine => console.log(color('[FFmpeg]'), commandLine))
                                .on('progress', progress => console.log(color('[FFmpeg]'), progress))
                                .on('end', () => console.log(color('[FFmpeg]'), 'Processing finished!'))
                ...
                if (isMedia && isAudio || isQuotedAudio || isVoice || isQuotedVoice) {
                        ...
                        const bass = await stream2Buffer(write => {
                            ffmpeg(buffer2Stream(mediaData))
                                .audioFilter('asetrate=44100*1.25,bass=g=3=f=8')
                                .format('mp3')
                                .on('start', commandLine => console.log(color('[FFmpeg]'), commandLine))
                                .on('progress', progress => console.log(color('[FFmpeg]'), progress))
                                .on('end', () => console.log(color('[FFmpeg]'), 'Processing finished!'))
                ...
                if (isMedia && isAudio || isQuotedAudio || isVoice || isQuotedVoice) {
                        ...
                        const bass = await stream2Buffer(write => {
                            ffmpeg(buffer2Stream(mediaData))
                                .audioFilter('atempo=1.1,asetrate=44100*0.7,bass=g=5')
                                .format('mp3')
                                .on('start', commandLine => console.log(color('[FFmpeg]'), commandLine))
                                .on('progress', progress => console.log(color('[FFmpeg]'), progress))
                                .on('end', () => console.log(color('[FFmpeg]'), 'Processing finished!'))
                ...
                case 'resep':
                    ...
                    break
                    case 'resend':
                        if (!isGroupAdmins) return piyo.reply(from, 'Gagal, Fitur ini hanya bisa digunakan oleh Admin', id)
                    if (quotedMsgObj) {
                        let encryptMedia
                        let replyOnReply = await piyo.getMessageById(quotedMsgObj.id)
                        ...
                        const mediaData = await decryptMedia(encryptMedia)
                        await piyo.sendFile(from, `data:${_mimetype};base64,${mediaData.toString('base64')}`, 'file', ':)', encryptMedia.id)
                    } else if (quotedMsgObj) {
                        let encryptMedia
                        let reponr = await piyo.getMessageById(quotedMsgObj.id)
                        ...
                        const _mimetype = encryptMedia.mimetype
                        const mediaData = await decryptMedia(encryptMedia)
                        await piyo.sendImageAsSticker(from, `data:${_mimetype};base64,${mediaData.toString('base64')}`, StickerMetadata)
                    } else piyo.reply(from, 'reply pesannya dong', id)
                    break
                case 'nhdl':
                ...
                case 'sspc':
                    ...
                    break
                case 'waktu':
                    piyo.reply(from, `*Waktu DenBot Aktif* \n\nPAGI = 07:00 - 10:00\n\nSIANG = 13:00 - 16:00\n\nMALAM = 19:00 - 22:00`, id)
                    break
                case prefix + 'listimg':
                ...
                kemtull += '╚═〘 *D E N  B O T* 〙'
                await piyo.reply(from, kemtull, id)
                break
                case prefix + 'listvn':
                    const vnlist = listvn
                let kemtul = `╔══✪〘 *List VN!* 〙✪══\n`
                ...
                kemtul += '╚═〘 *D E N  B O T* 〙'
                await piyo.reply(from, kemtul, id)
                break
                case 'listbacot':
                    const bacul = dbcot
                ...
                case 'getcode': {
                    if (!isOwnerBot) return await piyo.reply(from, ind.ownerOnly(), id)
                    if (!q) return await piyo.reply(from, `Silahkan ketik ${prefix}getcode harinya\nContoh: ${prefix}getcode 15`, id)
                    if (ar[0] === '15') {
                        await piyo.reply(from, ind.wait(), id)
             ...
             case 'sendsewa':
                ...
                case 'bcimg':
                    ...
                    if (isMedia && type === 'image' || isQuotedImage) {
                        ...
                        for (let grp of chaim) {
                            ...
                            if (!cukk.isReadOnly) piyo.sendFile(grp, `./media/images/bc.jpg`, 'piyo.jpg', `[DENBOT BROADCAST]\n\n${q}`, '', id)
                            if (cukk.isReadOnly) piyo.sendFile(grp, `./media/images/bc.jpg`, 'piyo.jpg', `[DENBOT BROADCAST]\n\n${q}`, '', id)
                        }
                ...
                case 'ban':
                    ...
                    break
                    case 'asupanukhty':
                    await piyo.sendFileFromUrl(from, `https://dapuhy-api.herokuapp.com/api/asupan/asupanukhty?apikey=${dapuhyapi}`, 'vid.mp4', '', id)
                    break
                    case 'asupan2':
                    await piyo.sendFileFromUrl(from, `https://dapuhy-api.herokuapp.com/api/asupan/asupanbocil?apikey=${dapuhyapi}`, 'asupan.mp4', '', id)
                    break
                    case 'asupan1':
                    await piyo.sendFileFromUrl(from, `https://dapuhy-api.herokuapp.com/api/asupan/asupansantuy?apikey=${dapuhyapi}`, 'vid.mp4', '', id)
                    break
                // AUTO STIKER
                case 'autostiker':
                case 'autostik':
            ...
            if ((!isCmd && chatId && isChat) && message.type === 'chat') {
                ...
                await piyo.reply(from, `DenBot Menjawab: ${piyoget.data.result}`, id)
            }
            // Simi-simi function
            if ((!isCmd && isGroupMsg && chatId && isSimi) && message.type === 'chat') {
                axios.get(`https://api.zeks.me/api/simi?apikey=${apizeks}&text=${encodeURIComponent(message.body)}`)
                    .then((res) => {
